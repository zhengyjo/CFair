select icu.*, charlson.charlson_comorbidity_index, apsiii,
--  SOFA,
ad.deathtime,
-- vaso_amount, 
rrt.rrt,
(weight_admit / ((0.01*height) * (0.01*height))) AS bmi
...
--get deathtime to the hour
inner join (SELECT subject_id, MAX(deathtime) as deathtime FROM physionet-data.mimic_core.admissions
group by subject_id) as ad
on ad.subject_id = icu.subject_id
...
-- left join (
--   select stay_id, avg(vaso_amount) as vaso_amount from
-- physionet-data.mimic_derived.vasopressin
-- group by stay_id --each stay has one vaso_amount
-- ) as vp
